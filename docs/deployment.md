...
```
installer/output/FastPassSetup-1.3.0.exe    (~131MB)
```
...
| 1.3.0 | 2026-03 | 클라이언트 PA (Trust Materials 기반 로컬 검증 + 서버 결과 보고), 전역 i18n, CA 인증서 웹 업로드, 언어 전환 버튼, 서비스 재시작, 버튼 디자인 통일, 성능 최적화 (캐시/병렬/비동기) |
