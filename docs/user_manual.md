...
| 속성 | 설명 | 기본값 |
|------|------|--------|
| `pa-api.base-url` | PA API 서버 URL | `http://localhost:8080` |
...
pa-api.base-url=http://pkd.smartcoreinc.com
...
1. Chrome/Edge 브라우저에서 `http://localhost:50000`에 접속합니다.
2. 주소창의 설치 아이콘을 클릭하거나, 메뉴에서 "앱 설치"를 선택합니다.
3. 독립 앱 창으로 실행됩니다.

### 12.2 캐싱 전략

- **정적 리소스** (CSS, JS, 이미지): 캐시 우선 (Cache First)
- **동적 페이지** (API 응답, HTML): 네트워크 우선 (Network First)
- **오프라인**: 네트워크 연결 불가 시 오프라인 페이지 표시

### 12.3 캐시 갱신

Service Worker 캐시 버전이 업데이트되면 다음 방문 시 자동으로 새 캐시가 적용됩니다. 수동으로 캐시를 갱신하려면:
